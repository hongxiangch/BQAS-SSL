...
def save_checkpoint_encoder(model, optimizer, epoch, loss, dim, dropout, seed, task, pretraining_num):
    """Saves a checkpoint."""
    # Record the state
    checkpoint = {
        'epoch': epoch,
        'loss': loss,
        'model_state': model.state_dict(),
        'encoder_state': model._encoder.state_dict(),
        'optimizer_state': optimizer.state_dict(),
        # 'encoder_state': model._encoder.state_dict()
    }
    # Write the checkpoint
    dir_name = f'pretrained/{task}/pretraining_{pretraining_num}/seed-{seed}/dim-{dim}'
    if not os.path.exists(dir_name):
        os.makedirs(dir_name)
    f_path = os.path.join(dir_name, f'model.pt')
    torch.save(checkpoint, f_path)
    return 0
...
def get_accuracy(inputs, targets):
    ...
    type_recon, qubit_recon, type_, qubit = ops_recon[:, :, 0:9], ops_recon[:, :, 9:], ops[:, :, 0:9], ops[:, :, 9:]
    # post processing, assume non-symmetric
    adj_recon, adj = adj_recon.triu(1), adj.triu(1)
    threshold = 0.5  # hard threshold
    qubit_recon_thre = qubit_recon > threshold

    correct_type = type_recon.argmax(dim=-1).eq(type_.argmax(dim=-1)).float().mean().item()
    correct_pos_qubit = qubit_recon_thre[qubit.type(torch.bool)].float().sum().item() / qubit.sum()
    fault_pos_qubit = qubit_recon_thre[~qubit.type(torch.bool)].float().sum().item()/(~qubit.type(torch.bool)).float().sum()
    ...
    correct_pos_adj = adj[adj_recon_thre].sum().item() / adj.sum()
    fault_pos_adj = adj_recon_thre[~adj.type(torch.bool)].float().sum().item()/(~adj.type(torch.bool)).float().triu(1).sum()

    # ops_correct = ops_recon.argmax(dim=-1).eq(ops.argmax(dim=-1)).float()
    # adj_correct = adj_recon_thre.eq(adj.type(torch.bool)).float()
    return correct_type, correct_pos_qubit, fault_pos_qubit, correct_pos_adj, fault_pos_adj
...
def get_val_acc_vae1(model, cfg, X_adj, X_ops, indices):
    ...
    X_ops_split = torch.split(X_ops, bs, dim=0)
    indices_split = torch.split(indices, bs, dim=0)
    correct_type_ave, correct_pos_qubit_ave, fault_pos_qubit_ave, correct_pos_adj_ave, fault_pos_adj_ave = 0, 0, 0, 0, 0
    for i, (adj, ops, ind) in enumerate(zip(X_adj_split, X_ops_split, indices_split)):
        adj, ops = adj.cuda(), ops.cuda()
        # preprocessing
        adj, ops, prep_reverse = preprocessing(adj, ops, **cfg['prep'])
        ...
        adj_recon, ops_recon = prep_reverse(adj_recon, ops_recon)
        adj, ops = prep_reverse(adj, ops)
        correct_type, correct_pos_qubit, fault_pos_qubit, correct_pos_adj, fault_pos_adj = get_accuracy((ops_recon, adj_recon), (ops, adj))
        correct_type_ave += correct_type * len(ind)/len(indices)
        correct_pos_qubit_ave += correct_pos_qubit * len(ind)/len(indices)
        fault_pos_qubit_ave += fault_pos_qubit * len(ind)/len(indices)
        correct_pos_adj_ave += correct_pos_adj * len(ind)/len(indices)
        fault_pos_adj_ave += fault_pos_adj * len(ind)/len(indices)

    return correct_type_ave, correct_pos_qubit_ave, fault_pos_qubit_ave, correct_pos_adj_ave, fault_pos_adj_ave
